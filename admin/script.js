...
(function () {
    ...
    function triggerFadeIn(container) {
        ...
        var items = container.querySelectorAll('.glass-card, .panel-header');
        items.forEach(function(el, i) {
            el.classList.remove('visible');
            el.classList.add('admin-fade-in');
            el.style.transitionDelay = (i * 0.08) + 's';
            // 用 rAF 确保 remove 生效后再添加
    ...
    navItems.forEach(function (item) {
        item.addEventListener('click', function () {
            ...
            // 触发新面板内的渐入动画
            if (panel) triggerFadeIn(panel);

            // 切换到消息时加载并启动轮询
            if (tab === 'messages') { loadMessages(true); startMsgPolling(); } else { stopMsgPolling(); }
            // 切换到 SMTP 时加载
    ...
    function createProjectCard(proj, index) {
        ...
        div.innerHTML =
            ...
            '  <input type="url" name="proj_link" value="' + escHtml((proj && proj.link) || '') + '" placeholder="https://">' +
            '</div>';
        ...
        saveProjectsBtn.addEventListener('click', function () {
            ...
            projectsEditor.querySelectorAll('.project-edit-card').forEach(function (card) {
                projects.push({
                    ...
                    link: card.querySelector('[name="proj_link"]').value.trim()
                });
            });
    ...
    var _msgPollInterval = 8000;

    function msgHash(data) {
        // 轻量指纹：消息数 + 首条ID + 首条已读状态 + 首条已回复状态
        if (!data || !data.length) return '0';
        var first = data[0];
        return data.length + ':' + (first.id || '') + ':' + (first.read ? '1' : '0') + ':' + (first.replied ? '1' : '0');
    }
    ...
    function loadMessages(force) {
        api('get_messages', null, 'GET').then(function (r) {
            if (!r.success) return;
            var msgs = r.data || [];
            var hash = msgHash(msgs);
            if (!force && hash === _lastMsgHash) return;
            _lastMsgHash = hash;
            var expandedIds = {};
            if (messagesList) {
                messagesList.querySelectorAll('.msg-card.expanded').forEach(function (card) {
                    expandedIds[card.getAttribute('data-id')] = true;
                });
            }
            renderMessages(msgs, expandedIds);
        });
    }

    function startMsgPolling() {
        stopMsgPolling();
        _msgPollTimer = setInterval(function () { loadMessages(false); }, _msgPollInterval);
    }
    ...
    function renderMessages(messages, expandedIds) {
        ...
        messages.forEach(function (msg) {
            ...
            // 图片预览（安全渲染：仅允许 data/ 下的相对路径图片）
            var imageHtml = '';
            if (msg.image && /^msg_images\/[a-zA-Z0-9_.\-]+\.(jpg|jpeg|png|gif|webp)$/.test(msg.image)) {
                imageHtml = '<div class="msg-attachment-label"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg> 附件图片（点击查看大图）</div><div class="msg-image"><img src="../data/' + escHtml(msg.image) + '" alt="附图" loading="lazy" referrerpolicy="no-referrer"></div>';
            }
            html +=
                ...
                '      <span class="msg-email">' + escHtml(msg.email) + '</span> ' +
                       statusHtml +
                (msg.image ? ' <span class="msg-has-image" title="含图片">🖼</span>' : '') +
                '    </div>' +
                ...
                (imageHtml ? '  <div class="msg-attachment">' + imageHtml + '</div>' : '') +
        ...
        messagesList.querySelectorAll('.msg-card').forEach(function (card) {
            ...
            if (expandedIds[cid]) card.classList.add('expanded');
        });
    ...
    // 事件委托：统一处理消息列表的点击事件（避免每次渲染重新绑定）
    if (messagesList) {
        messagesList.addEventListener('click', function (e) {
            var target = e.target;

            // 灯箱预览
            var imgEl = target.closest('.msg-image img');
            if (imgEl) {
                e.stopPropagation();
                var lb = document.createElement('div');
                lb.className = 'msg-lightbox';
                lb.innerHTML = '<img src="' + escHtml(imgEl.src) + '" alt="预览">';
            ...
            // 回复按钮
            var replyBtn = target.closest('.msg-reply-btn');
            if (replyBtn) {
                e.stopPropagation();
                openReplyModal(replyBtn.getAttribute('data-id'), replyBtn.getAttribute('data-email'), replyBtn.getAttribute('data-name'));
                return;
            }

            // 删除按钮
            var deleteBtn = target.closest('.msg-delete-btn');
            if (deleteBtn) {
                e.stopPropagation();
                if (!confirm('确认删除这条消息？')) return;
                api('delete_message', { id: deleteBtn.getAttribute('data-id') }).then(function (r) {
                    toast(r.message, r.success ? 'success' : 'error');
                    if (r.success) loadMessages(true);
                });
                return;
            }
            ...
            // 展开/收起消息卡片
            var card = target.closest('.msg-card');
            if (card) {
                card.classList.toggle('expanded');
                if (card.classList.contains('unread')) {
                    card.classList.remove('unread');
                    api('mark_read', { id: card.getAttribute('data-id') });
                }
            }
        });
    }
    ...
    if (refreshMsgsBtn) {
        refreshMsgsBtn.addEventListener('click', function () { loadMessages(true); });
    }
    ...
    // 回复附图
    var replyImageFile = document.getElementById('replyImageFile');
    var replyImageAdd = document.getElementById('replyImageAdd');
    var replyImageThumb = document.getElementById('replyImageThumb');
    var replyImagePreviewImg = document.getElementById('replyImagePreviewImg');
    var replyImageRemove = document.getElementById('replyImageRemove');
    var replyImageData = '';

    if (replyImageAdd) {
        replyImageAdd.addEventListener('click', function () { replyImageFile.click(); });
    }
    if (replyImageFile) {
        replyImageFile.addEventListener('change', function () {
            var file = this.files[0];
            if (!file) return;
            if (!file.type.match(/^image\/(jpeg|png|gif|webp)$/)) { toast('仅支持 JPG/PNG/GIF/WebP', 'error'); return; }
            if (file.size > 5 * 1024 * 1024) { toast('图片不能超过 5MB', 'error'); return; }
            var reader = new FileReader();
            reader.onload = function (e) {
                replyImageData = e.target.result;
                replyImagePreviewImg.src = replyImageData;
                replyImageThumb.style.display = '';
                replyImageAdd.style.display = 'none';
            };
            reader.readAsDataURL(file);
        });
    }
    if (replyImageRemove) {
        replyImageRemove.addEventListener('click', function () {
            replyImageData = '';
            replyImagePreviewImg.src = '';
            replyImageThumb.style.display = 'none';
            replyImageAdd.style.display = '';
    ...
    function clearReplyImage() {
        replyImageData = '';
        if (replyImagePreviewImg) replyImagePreviewImg.src = '';
        if (replyImageThumb) replyImageThumb.style.display = 'none';
        if (replyImageAdd) replyImageAdd.style.display = '';
        if (replyImageFile) replyImageFile.value = '';
    }
    ...
    if (replyForm) {
        replyForm.addEventListener('submit', function (e) {
            ...
            if (replyImageData) payload.image = replyImageData;

            api('reply_message', payload).then(function (r) {
    ...
    if (activePanel) triggerFadeIn(activePanel);
    ...
    _badgePollTimer = setInterval(function () {
        ...
        if (activeTab && activeTab.getAttribute('data-tab') === 'messages') return;
    ...
    document.addEventListener('visibilitychange', function () {
        var isMessages = document.querySelector('.nav-item.active');
        if (!isMessages || isMessages.getAttribute('data-tab') !== 'messages') return;
        if (document.hidden) {
            stopMsgPolling();
        } else {
            loadMessages(false);
